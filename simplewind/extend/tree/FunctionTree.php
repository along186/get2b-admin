...
class FunctionTree
{
    ...
    public function getParent($myId)
    {
        ...
        if (!isset($this->arr[$myId]))
            return false;
        $pid = $this->arr[$myId]['category_id'];
        $pid = $this->arr[$pid]['category_id'];
...
//        print_r($this->arr);die;
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                $newArr[$a['cid']][$a['fid']] = $a;
                if(!in_array($a['cname'],$nameArr)) {
                    $nameArr[$a['cid']] = $a['cname'];
                }
            }
        }

        return [$newArr,$nameArr];
    }
     ...
     * @return string
     */
    public function getTree($currentFunctionIds)
    {
        list($arr, $nameArr) = $this->getArr();
        $str = "";
        foreach ($arr as $cid => $functions) {
            $str .= "<tr class='data-item-tr'><td></td><td>".$cid."</td><td>".$nameArr[$cid]."</td></tr>";
            foreach ($functions as $fid => $row) {
                if (in_array($fid, $currentFunctionIds)) {
                    $str .= "<tr class='data-item-tr'><td><input type='checkbox' class='js-check' data-yid='js-check-y' data-xid='js-check-x' name='ids[]' value= '".$fid."' data-name='".$row['fname']."' checked ></td><td>".$fid."</td><td>".$this->nbsp.$this->nbsp.$this->icon[1].$row['fname']."</td></tr>";
                } else {
                    $str .= "<tr class='data-item-tr'><td><input type='checkbox' class='js-check' data-yid='js-check-y' data-xid='js-check-x' name='ids[]' value= '".$fid."' data-name='".$row['fname']."'></td><td>".$fid."</td><td>".$this->nbsp.$this->nbsp.$this->icon[1].$row['fname']."</td></tr>";
                }
            }
        }
        return $str;
