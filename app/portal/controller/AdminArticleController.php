...
class AdminArticleController extends AdminBaseController
{
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            $data['post']['support_platform'] = implode(',', $data['post']['support_platform']);
            $data['post']['training_mode'] = implode(',', $data['post']['training_mode']);
            $data['post']['technical_support'] = implode(',', $data['post']['technical_support']);
    ...
    public function edit()
    {
        ...
        $themeModel        = new ThemeModel();
        $articleThemeFiles = $themeModel->getActionThemeFiles('portal/Article/index');
        $supportPlatform = explode(',',$post->support_platform);
        $trainingMode = explode(',',$post->training_mode);
        $technicalSupport = explode(',',$post->technical_support);

        $functionList = Db::table('gb_portal_function')->where('id in ('.$post->functions.') and status = 1')->select()->toArray();
        $functionName = array_column($functionList,'name');

        $this->assign('article_theme_files', $articleThemeFiles);
        $this->assign('post', $post);
        $this->assign('functionName', implode(' ', $functionName));
        $this->assign('supportPlatform', $supportPlatform);
        $this->assign('trainingMode', $trainingMode);
        $this->assign('technicalSupport', $technicalSupport);
